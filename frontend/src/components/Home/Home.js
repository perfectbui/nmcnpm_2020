...
const Home = (props) => {
	...
	useEffect(() => {
		...
		Axios({
			...
			.then((response) => {
				response.data.posts[0].usersTaken = response.data.posts[0].usersTaken.map((item) => JSON.parse(item));
		...
		<div className="home">
			...
			<div className="content">
				...
				<div style={{ marginTop: '20px' }}>
					{/* {dataPost ? dataPost.userTaken.map(user => )} */}
					{dataPost && dataPost[0].usersTaken ? <table>
						<tr>
							<th>User</th>
							<th>Time</th>
							<th>Score</th>
						</tr>
						{dataPost[0].usersTaken.map(user => <tr>
							<td>{user.name}</td>
							<td>{dateFormat(user.date, "dddd, mmmm dS, yyyy")}</td>
							<td>{user.score}</td>
						</tr>)}
					</table> : null}
				</div>
